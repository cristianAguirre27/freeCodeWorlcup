...
echo "$($PSQL "select name from teams full join games on games.winner_id = teams.team_id where year=2014 and round='Eighth-Final' union select name from teams full join games
on games.opponent_id = teams.team_id where year = 2014 and round='Eighth-Final'")"
